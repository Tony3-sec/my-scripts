'''
Send DNS query packet with spoofed source IP
Tested on CentOS 6
'''
...
class DnsPacketBuilder:
	...
	def build_packet(self, url):
		dns_data = struct.pack("!H", 12049)  # Transaction Ids
		dns_data += struct.pack("!H", 256)  # Flags
		dns_data += struct.pack("!H", 1)  # Questions
		dns_data += struct.pack("!H", 0)  # Answers
		dns_data += struct.pack("!H", 0)  # Authorities
		dns_data += struct.pack("!H", 0)  # Additional
		split_url = url.split(".")
		for part in split_url:
			dns_data += struct.pack("!B", len(part))
			for byte in bytes(part):
				dns_data += struct.pack("!c", byte)
		dns_data += struct.pack("!B", 0)  # End of String
		dns_data += struct.pack("!H", 1)  # Query Type (A=1, NS=2, CNAME=5, SOA=6, PTR=120, HINFO=121, MX=123, AAAA=125, TXT=16, ANY=255)
		dns_data += struct.pack("!H", 1)  # Query Class (IN(INTERNET)=1, CSNET(COMPUTER SCIENCE NETWORK)=2)
		return dns_data

builder = DnsPacketBuilder()
dns_data = builder.build_packet("www.google.com")
# create raw socket
try:
...
packet = ''
source_ip = '192.168.12.252' # spoof source IP if you want
dest_ip = '192.168.10.252'
...
udp_header_len = 8
user_dgram_len = udp_header_len + len(dns_data)
...
packet = ip_header + udp_header + dns_data
